...
function EditableSet({ set, validationPrefix, validationKey, onChange, onDelete, onMove, canDelete, editing, error, requestConfirmation }: { set: SessionSet; validationPrefix: string; validationKey?: string; onChange: (next: SessionSet) => void; onDelete: () => void; onMove: (direction: -1 | 1) => void; canDelete: boolean; editing: boolean; error?: string; requestConfirmation: (request: ConfirmationRequest) => void }) {
  ...
  return <div className="grid gap-3 border-b border-line-subtle py-4 tablet:grid-cols-[3rem_minmax(6rem,1fr)_6rem_8rem_7rem_auto] tablet:items-end">
    ...
    <div className="grid gap-1 text-xs text-ink-muted"><label htmlFor={`weight-${set.id}`}>น้ำหนัก</label><div className="flex"><input id={`weight-${set.id}`} data-history-validation={weightInvalid ? validationKey : undefined} aria-invalid={weightInvalid || undefined} disabled={!editing} aria-label={`Set ${set.sequence} weight`} type="number" step="0.1" min="0" value={actualWeight} onChange={(event) => { const value = readNumberInput(event.currentTarget); onChange({ ...set, actualWeight: value === "" ? null : { value: Number(value), unit, kg: kgFromWeight(Number(value), unit) } }); }} className="min-h-11 min-w-0 flex-1 border border-line bg-surface px-3 text-ink disabled:opacity-70" /><select disabled={!editing} aria-label={`Set ${set.sequence} weight unit`} value={unit} onChange={(event) => { const nextUnit = event.target.value as WeightUnit; onChange({ ...set, actualWeight: set.actualWeight ? { ...set.actualWeight, unit: nextUnit, kg: kgFromWeight(set.actualWeight.value, nextUnit) } : null }); }} className="min-h-11 border-y border-r border-line bg-surface px-2 text-ink disabled:opacity-70"><option value="KG">KG</option><option value="LB">LB</option></select></div></div>
    <label className="grid gap-1 text-xs text-ink-muted">Reps<input data-history-validation={repsInvalid ? validationKey : undefined} aria-invalid={repsInvalid || undefined} disabled={!editing} aria-label={`Set ${set.sequence} reps`} type="number" min="1" step="1" value={actualReps} onChange={(event) => { const value = readNumberInput(event.currentTarget); onChange({ ...set, actualReps: value === "" ? null : Number(value) }); }} className="min-h-11 border border-line bg-surface px-3 text-ink disabled:opacity-70" /> </label>
    ...
    <label className="grid gap-1 text-xs text-ink-muted">ค่า<input disabled={!editing} aria-label={`Set ${set.sequence} effort value`} type="number" min="0" max="10" step={set.actualEffort?.metric === "RIR" ? "1" : "0.5"} value={set.actualEffort?.value ?? ""} onChange={(event) => { const value = readNumberInput(event.currentTarget); onChange({ ...set, actualEffort: set.actualEffort ? { ...set.actualEffort, value: value === "" ? 0 : Number(value) } : null }); }} className="min-h-11 border border-line bg-surface px-3 text-ink disabled:opacity-70" /> </label>
