...
const week = {
  ...
  days: [{ id: "day-1", routine_day_id: "routine-day-1", template_id: "template-1", display_order: 1, day_label: "Push", template_name: "Push", completed_count: 2, active_count: 1 }],
};
...
describe("SupabaseRoutineTrackingRepository", () => {
  it("parses independent Frequency and Coverage totals", () => {
    expect(parseRoutineWeek(week)).toMatchObject({
      frequencyActual: 3,
      frequencyTarget: 3,
      coverageActual: 2,
      coverageTarget: 3,
      days: [expect.objectContaining({ activeCount: 1 })],
    });
  });
  ...
  it("initializes timezone before loading a weekly view", async () => {
    const currentWeek = {
      timezone: "Asia/Bangkok",
      current_week_start: "2026-08-17",
      next_week_start: "2026-08-24",
      current_plan: week,
      scheduled_activation: null,
    };
    const client = new FakeClient(["Asia/Bangkok", currentWeek]);

    await new SupabaseRoutineTrackingRepository(client).getCurrentWeek();

    expect(client.calls.map((call) => call.path)).toEqual([
      "rpc/preferences_get_or_create_timezone",
      "rpc/routine_get_current_week",
    ]);
  });
});
